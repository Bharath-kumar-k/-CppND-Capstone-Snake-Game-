...
void Renderer::Render(Snake const snake,Snake const l_snake, SDL_Point const &food, std::vector<SDL_Point> const &obstruction) {
  SDL_Rect block;
  ...
  // Render l_snake's body
  SDL_SetRenderDrawColor(sdl_renderer, 0xFF, 0xFF, 0xFF, 0xFF);
  for (SDL_Point const &point : l_snake.body) {
    block.x = point.x * block.w;
    block.y = point.y * block.h;
    SDL_RenderFillRect(sdl_renderer, &block);
  }

  // Render l_snake's head
  block.x = static_cast<int>(l_snake.head_x) * block.w;
  block.y = static_cast<int>(l_snake.head_y) * block.h;
  if (l_snake.alive) {
    SDL_SetRenderDrawColor(sdl_renderer, 0x00, 0x7A, 0xCC, 0xFF);
  } else {
    SDL_SetRenderDrawColor(sdl_renderer, 0xFF, 0x00, 0x00, 0xFF);
